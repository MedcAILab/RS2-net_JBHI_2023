...
def train_step_cls_seg(train_loader, model, epoch, optimizer, criterion, args):
    ...
    for step, (imagesA, mask, labels) in enumerate(train_loader):
        ...
        loss_x = criterion(out_A, labels)
        loss_seg = criterion(torch.squeeze(out_Seg, dim=0), maskA)
        lossValue = loss_x + loss_seg
        ...
        # plot progress
        bar_str = '{} / {} | Time: {batch_time:.2f} mins | Loss: {loss:.4f} | Loss_cla: {loss_cla:.4f} | Loss_seg:{loss_seg:.4f} \n '
        bar.suffix = bar_str.format(step + 1, iters_per_epoch, batch_time=batch_time * (iters_per_epoch - step) / 60,
                                    loss=lossValue.item(), loss_cla=loss_x.item(), loss_seg=loss_seg.item())
        bar.next()
...
def train_step_cls(train_loader, model, epoch, optimizer, criterion, args):
    ...
    for step, (imagesA, mask, labels) in enumerate(train_loader):
        ...
        # bar.suffix = bar_str.format(step + 1, iters_per_epoch, batch_time=batch_time * (iters_per_epoch - step) / 60,
        #                             loss=lossValue.item(), loss_cla=loss_x.item(), loss_seg=loss_seg.item())
        bar_str = '{} / {} | Time: {batch_time:.2f} mins | Loss_cla: {loss:.4f} \n '
        bar.suffix = bar_str.format(step + 1, iters_per_epoch, batch_time=batch_time * (iters_per_epoch - step) / 60,
                                    loss=lossValue.item())
        bar.next()
