...
class SKNet(nn.Module):
    ...
    def forward(self, x): # 1 3 256 256
        fea = self.basic_conv(x) # 1 64 128 128
        fea = self.maxpool(fea) # 1 64 64 64
        fea = self.stage_1(fea) # 1 256 64 64
        fea = self.stage_2(fea) # 1 512 32 32
        fea = self.stage_3(fea) # 1 1024 16 16
        fea = self.stage_4(fea) # 1 2048 8 8
        fea = self.gap(fea) # 1 2048 1 1
        # fea = torch.squeeze(fea) # 2048
        fea = fea.view(fea.size(0), -1) # 1 2048
        fea = self.classifier(fea) # 2  # 1 2
        fea = self.sigmd(fea)  # 2  # 1 2
        return fea
